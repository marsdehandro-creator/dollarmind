...
import type { RawFileStore } from './interfaces/RawFileStore.js';
...
import { extractDocumentText, type ExtractionAdapters } from '../ingestion/extract.js';
...
export class LocalStatementImportService implements StatementImportService {
  /**
   * Takes already-loaded parser rules and a RawFileStore rather than reading
   * config files / writing to disk itself, so this class stays portable
   * across Node (fs-backed) and the browser/on-device (blob-backed) — see
   * docs/v2-migration-spec.md's core principle.
   */
  constructor(
    private readonly deps: StatementServiceDeps,
    private readonly rules: StatementParserRules,
    private readonly fileStore: RawFileStore,
    private readonly extractionAdapters: ExtractionAdapters,
  ) {}
  ...
  async uploadStatement(input: { tenantId: string; file: UploadedFile }): Promise<StatementUploadResult> {
    ...
    const docId = newId();
    const { filePath, blobData } = await this.fileStore.save(docId, safeName, file.buffer);
